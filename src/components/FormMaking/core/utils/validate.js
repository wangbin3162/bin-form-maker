...
export function isField(str) {
  const reg = /(^([a-zA-Z]_?)*$)|(^[a-zA-Z](_?[a-zA-Z0-9])*_?$)/
  return reg.test(str)
}

// 匹配以字母开头，或其余数字中划线组成
export function isFieldNotContainer_(str) {
  const reg = /(^([a-zA-Z]-?)*$)|(^[a-zA-Z](-?[a-zA-Z0-9])*-?$)/
  return reg.test(str)
}

// 匹配以小写字母开头，或其余数字下划线组成
export function isFieldLetter(str) {
  const reg = /(^([a-z]_?)*$)|(^[a-z](_?[a-z0-9])*_?$)/
  return reg.test(str)
}
...
// 中文、字母数字下划线的字符串
export function isTitleNumZM(str) {
...
// 用户密码合法性校验
export function isLegalPwd(str) {
  const reg = /^(?=.*[0-9])(?=.*[a-zA-Z])(?=.*[^a-zA-Z0-9]).{8}$/
  return reg.test(str)
}
